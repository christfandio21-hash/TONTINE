...
export default function LoansPage({ tontineId, isAdmin }: LoansPageProps) {
  ...
  const handleRepay = async (loanId: string) => {
    ...
    // Split payment: first cover interest, then principal (or proportional)
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-slate-50">
          ...
          {loans.map((l) => {
            ...
            const interestRemaining = Math.max(0, totalInterestForLoan - totalInterestPaid);
            const previewRepayInterest = showRepay === l.id ? Math.min(repayTotal, interestRemaining) : 0;
            const previewRepayPrincipal = showRepay === l.id ? Math.max(0, repayTotal - previewRepayInterest) : 0;
